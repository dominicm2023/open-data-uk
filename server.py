...
def lab_orgweb() -> HTMLResponse:
    ...
    path = Path(__file__).parent / "orgweb.html"
    if not path.exists():
        return HTMLResponse(pagerender.render_missing(None), status_code=404,
                            headers={"Cache-Control": "no-store"})
    return HTMLResponse(path.read_text(encoding="utf-8"),
...
def lab_organograms() -> HTMLResponse:
    ...
    path = Path(__file__).parent / "organograms.html"
    if not path.exists():
        return HTMLResponse(pagerender.render_missing(None), status_code=404,
                            headers={"Cache-Control": "no-store"})
    return HTMLResponse(path.read_text(encoding="utf-8"),
...
@app.get("/lab/scroll", include_in_schema=False)
def lab_scroll() -> HTMLResponse:
    """The scrollytelling tests. Under /lab, private by path prefix."""
    from pathlib import Path

    path = Path(__file__).parent / "scrolly.html"
    if not path.exists():
        return HTMLResponse(pagerender.render_missing(None), status_code=404,
                            headers={"Cache-Control": "no-store"})
    return HTMLResponse(path.read_text(encoding="utf-8"),
...
@app.get("/lab/rogues", include_in_schema=False)
def lab_rogues() -> HTMLResponse:
    """The rogue-joins investigations. Under /lab, private by path prefix."""
    from pathlib import Path

    path = Path(__file__).parent / "rogues.html"
    if not path.exists():
        return HTMLResponse(pagerender.render_missing(None), status_code=404,
                            headers={"Cache-Control": "no-store"})
    return HTMLResponse(path.read_text(encoding="utf-8"),
...
@app.get("/lab/strands", include_in_schema=False)
def lab_strands() -> HTMLResponse:
    """The comparison strands. Under /lab, private by path prefix."""
    from pathlib import Path

    path = Path(__file__).parent / "strands.html"
    if not path.exists():
        return HTMLResponse(pagerender.render_missing(None), status_code=404,
                            headers={"Cache-Control": "no-store"})
    return HTMLResponse(path.read_text(encoding="utf-8"),
